 ...
 */
class Eyrina : public Application_thread, public Loggable
{
private:
    ...
    const map<string, gcode_settings> g_code_commands = {
        { "G0",  { &Eyrina::G_code_G0,  { 'X', 'Y', 'Z', 'T', 'R', 'F', 'P', 'H' }, {   } } },
        { "G1",  { &Eyrina::G_code_G1,  { 'X', 'Y', 'Z', 'T', 'R', 'F', 'P', 'H' }, {   } } },
        { "G2",  { &Eyrina::G_code_G2,  { 'X', 'Y', 'Z', 'T', 'R', 'F', 'P', 'H' }, {   } } },
        { "G28", { &Eyrina::G_code_G28, {   }, { 'X','Y', 'Z', 'T', 'R', 'F', 'P', 'H' } } },
        { "M0",  { &Eyrina::G_code_M0,  {   }, { 'X','Y', 'Z', 'T', 'R', 'F', 'P', 'H' } } },
        { "M10", { &Eyrina::G_code_M10, {   }, { 'X','Y', 'Z', 'T', 'R', 'F', 'P', 'H' } } },
     ...
     */
    vector<L6470 *> stepper_drivers{1};

    /**
     * @brief  Axis which can b controlled from application
     */
    map<char, Motion_axis_L6470*> axis;
...
public:
     ...
     * @return int  0 is all is OK, otherwise errno
     */
    virtual int Thread(vector<string> &args) final override;
     ...
     */
    int Add_axis(char axis_name, Motion_axis_L6470 *new_axis);
    ...
    /**
     * @brief Perform diagnostic on all motors and axis
     *
     * @return int  Number of errors in system
     */
    int Diagnostic();
...
private:
    ...
    /**
     * @brief Initialize GPIO expanders and define pins, which are used as chip selects for SPI stepper motor driver
     *
     * @return vector<Pin*> Pins on expanders used as chip selects
     */
    vector<Pin*> Setup_GPIO_expanders();

    /**
     * @brief   Setups stepper motor and config then according toi configuration structire
     *
     * @param spi           SPI master on MCU
     * @param chip_select   Vector of chip select pins of GPIO expanders
     * @param motor_settings    Configuration for stepper motors, order must be same as chip select pins
     */
    void Setup_motors(SPI_master *spi, vector<Pin *> &chip_select, const std::array<Eyrina_config::motor_config, 8> &motor_settings);

    void Setup_axis(const std::array<Eyrina_config::axis_config, 8> axis_settings);
    ...
    /**
     * @brief Run axis at given speed
     *
     * @param params    Names and speed of axis, which will be running
     * @param flags     Always empty, use only params
     * @return int      Number of axis which was been really run (not counting axis, which is not enabled or etc.)
     */
    int G_code_G2(map<char, double> &params, vector<char> &flags);
